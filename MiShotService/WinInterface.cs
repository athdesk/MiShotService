...
namespace MiShotService
{
    class WinInterface
    {
        ...
        #endregion
        private static string RegCmdLine => Assembly.GetEntryAssembly().Location + " " + Program.ARG_STANDALONE;
        public static void SetStartup(bool AutoStart)
        {
            ...
            if (AutoStart)
                RunKey.SetValue(Program.DISPLAY_NAME, RegCmdLine);
            else
                RunKey.DeleteValue(Program.DISPLAY_NAME, false);
        }
        ...
        public static bool IsOnStartup()
        {
            ...
            var RunValue = RunKey.GetValue(Program.DISPLAY_NAME);
            if (RunValue == null)
                return false;
            else
